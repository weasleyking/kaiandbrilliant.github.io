const interact6 = (p) => {
  ...
  p.preload = () => {
    // Load the images
    img1 = p.loadImage('assets/image7.jpg');
    img2 = p.loadImage('assets/image8.jpg');
    img3 = p.loadImage('assets/image9.jpg');
  };

  p.setup = () => {
    const sketchContainer = p.select('#interact6');
  ...
  p.draw = () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (i === 0) {
        ...
        text = 'to make a vioin';
      } else if (i === 1) {
        x = 3.2 * p.width / 4;
        y = 1.2 * p.height / 4;
        img = img2;
        lerpFactor = moveFactor2
        text = 'Pacing Time in VR';
      } else {
        x = p.width / 2;
        y = 3 * p.height / 4;
        img = img3;
        lerpFactor = moveFactor3
        text = '0 G Flight Mechanical Artifact';
      }
